...
- `demo_database.sqlite` - ready-to-open SQLite database generated from `seed.sql`.
- `seed.sql` - canonical SQLite schema and seed data.
- `demo_suppliers.json` - JSON export of the same demo supplier catalog for app or backend use.
- `example_queries.sql` - sample SQLite statements for supplier matching, request storage, AI report storage, and lead submission.
...
Open `demo_database.sqlite` with any SQLite client.

To rebuild the database from the SQL seed:

```bash
sqlite3 demo_database.sqlite < seed.sql
```
...
`example_queries.sql` contains ready-to-adapt SQL for the matching flow. Keep `seed.sql`, `demo_database.sqlite`, and `demo_suppliers.json` in sync when the dataset changes.
